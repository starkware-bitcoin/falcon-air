...
impl Claim {
    ...
    pub fn gen_trace(
        ...
        let remainders = poly
            ...
            .collect::<Vec<_>>();
        let squares = remainders.iter().map(|a| a * a).collect::<Vec<_>>();
        let mut cum_sum = Vec::with_capacity(POLY_SIZE);
        squares.iter().fold(0, |acc, x| {
            cum_sum.push(acc + x);
            acc + x
        });
...
pub struct Eval {
    ...
    pub half_rc_lookup_elements: range_check::LookupElements,
    /// Lookup elements for f_ntt
    pub input_lookup_elements: Option<sub::LookupElements>,
}
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: stwo_constraint_framework::EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let [cum_sum_last, cum_sum_current] =
        ...
        if let Some(input_lookup_elements) = &self.input_lookup_elements {
            eval.add_to_relation(RelationEntry::new(
        ...
        eval.add_constraint(
            coeff.clone() + borrow.clone() * (E::F::from(M31(Q)) - coeff.clone() - coeff.clone())
                - remainder.clone(),
        );

        eval.add_constraint(
            is_not_first * (cum_sum_last + remainder.clone() * remainder - cum_sum_current),
        );
...
impl InteractionClaim {
    ...
    pub fn gen_interaction_trace(
        ...
        half_rc_lookup_elements: &range_check::LookupElements,
        input_lookup_elements: &Option<sub::LookupElements>,
    ) -> (
        ColumnVec<CircleEvaluation<SimdBackend, M31, BitReversedOrder>>,
        InteractionClaim,
    ) {
        ...
        if let Some(input_lookup_elements) = &input_lookup_elements {
            let mut col_gen = logup_gen.new_col();
            for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
                let result_packed = trace[0].data[vec_row];
                let denom: PackedQM31 = input_lookup_elements.combine(&[result_packed]);
                let numerator = PackedQM31::one();
                col_gen.write_frac(vec_row, numerator, denom);
            }
            col_gen.finalize_col();
