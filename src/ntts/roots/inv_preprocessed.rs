...
use crate::{big_air::relation::InvRootsLookupElements, ntts::ROOTS, zq::inverses::INVERSES_MOD_Q};
...
impl InvRoots {
    ...
    pub fn gen_column_simd(
        ...
        let mut roots = ROOTS[self.poly_log_size - 1]
            .iter()
            .map(|x| INVERSES_MOD_Q[*x as usize])
            .collect_vec();
    ...
    pub fn id(&self) -> PreProcessedColumnId {
        PreProcessedColumnId {
            id: format!("inv_roots_of_unity_{}", self.poly_log_size),
        }
    }
}
...
pub struct Eval {
    ...
    pub lookup_elements: InvRootsLookupElements,
    pub poly_log_size: usize,
}
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let j = eval.get_preprocessed_column(InvRoots::new(self.poly_log_size).id());
        let mut inv_root_id = InvRoots::new(self.poly_log_size).id();
        inv_root_id.id.push_str("_inv_root");
        let inv_root = eval.get_preprocessed_column(inv_root_id);
...
impl InteractionClaim {
    ...
    pub fn gen_interaction_trace(
        ...
        lookup_elements: &InvRootsLookupElements,
        poly_log_size: usize,
    ) -> (
        ...
        let js_and_roots = InvRoots::new(poly_log_size).gen_column_simd();
