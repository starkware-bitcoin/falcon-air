    ...
    pub fn gen_interaction_trace(
        ...
        for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
            ...
            for num in result_packed.to_array() {
                ...
                vals[0] = num;
                let packed = PackedM31::from_array(vals);

                // Create the denominator using the lookup elements
                let denom: PackedQM31 = f_ntt_lookup_elements.combine(&[packed]);
        ...
        for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
            ...
            for num in result_packed.to_array() {
                ...
                // Create the denominator using the lookup elements
                let denom: PackedQM31 = g_ntt_lookup_elements.combine(&[packed]);
        ...
        for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
            ...
            for num in result_packed.to_array() {
                ...
                // Create the denominator using the lookup elements
                let denom: PackedQM31 = mul_lookup_elements.combine(&[packed]);
